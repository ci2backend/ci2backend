...
class T_sub_menu extends MY_Model 
{

	protected $_table_name = 'sub_menus'; // table name
	...
	public function get_parent_by_action_router($select = '*', $action_router)
	{

		$this->load->model('t_menu');

		$target_table = $this->t_menu->table_name;

		$conditions = $this->table_name . '.' . 'menu_id' . '=' . $target_table . '.id';
		...
		$menus = $this->get_data_by_property($select, array(
				 $this->table_name.'.action_router' => $action_router
			)
		);
