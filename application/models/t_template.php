...
class T_template extends MY_Model 
{

	protected $_table_name = 'templates'; // table name
	...
	public $rules_create = array(
		array(
            'field' => 'template_key', 
            'label' => 'lang:Template_key', 
            'rules' => 'trim|required|max_length[100]|is_unique[templates.template_key]|regex_match[/^[a-zA-Z_\s]+$/]'
        ),
        array(
            'field' => 'template_name', 
	...
	public function get_extension_by_template($select = '*', $where = array())
	{

		$this->load->model('t_extend_loader');

		$target_table = $this->t_extend_loader->table_name;

		$conditions = $this->table_name . '.' . 'template_key' . '=' . $target_table . '.template_key';

		$left = 'inner';

		$this->db->join($target_table, $conditions, $left);

		return $this->get_data_by_property($select, $where);
